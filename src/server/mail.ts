    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:${MBT_BRAND.surface};border:1px solid ${MBT_BRAND.line};border-radius:20px;overflow:hidden;">
        <tr><td style="padding:28px 28px 12px 28px;">
          <div style="font-family:ui-monospace,Menlo,'SF Mono',monospace;font-size:11px;letter-spacing:0.2em;color:${MBT_BRAND.lime};font-weight:900;">● BASE</div>
        </td></tr>
        ${innerHtml}
...
export function inviteMail({
  ...
  return {
    ...
    subject: therapistName
      ? `${therapistName} heeft je uitgenodigd voor BASE`
      : 'Welkom bij BASE',
    html: layout(inner),
    text:
      `Hallo ${recipientName},\n\n` +
      `${therapistName ? `${therapistName} heeft je uitgenodigd` : 'Je bent uitgenodigd'} voor BASE.\n\n` +
