    ...
    <div className="min-h-screen" style={{ background: P.bg, color: P.ink }}>
      <div className="max-w-4xl mx-auto px-4 pt-10 pb-24 space-y-6">
        ...
        <Tile>
          ...
          <DarkSelect className="mt-2" value={selectedPatientId} onChange={(e) => setSelectedPatientId(e.target.value)}>
            <option value="">— kies patiënt —</option>
            {patientOptions.map((p) => (
              <option key={p.id} value={p.id}>{p.label}</option>
            ))}
        ...
        {selectedPatientId && (
          <div className="flex flex-col gap-2">
            ...
            {analyses.map((a) => {
              ...
              return (
                <Tile key={a.id} accentBar={accent} href={`/therapist/assessments/hardloopanalyse/${a.id}`}>
                  <div className="flex items-start justify-between gap-3 flex-wrap">
